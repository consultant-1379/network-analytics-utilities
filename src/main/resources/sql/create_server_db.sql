/*
 * Copyright (c) 2019-2023 Spotfire AB,
...
create table SN_VERSION (
    ...
    UPGRADE_INITIATED timestamp(0),
    UPGRADE_INFO varchar(400)
);
...
create table USERS (
    ...
    GIVEN_NAME varchar(450) null,
    FAMILY_NAME varchar(450) null,    
...
alter table DEP_DISTRIBUTION_CONTENTS 
    add constraint FK_DDC_USR foreign key (
        PUBLISHED_BY) 
    references USERS (
...
create table LIB_DATA (
    ...
    COMPRESSED_CONTENT_SIZE bigint null,
    DIGEST bytea null,
    DATA oid not null,
    constraint PK_LIB_DATA primary key (DATA_ID),
...
create table LIB_ITEM_VERSIONS (
     ...
     RESTORED_FROM_ID char(36) null,
     RESTORED_FROM_TIMESTAMP timestamp(6) null,
     constraint PK_LIB_ITEM_VERSIONS primary key (VERSION_ID),
     constraint LIB_ITEM_VERSIONS_FK1 foreign key (DATA_ID)
         references LIB_DATA (DATA_ID),
     constraint LIB_ITEM_VERSIONS_FK2 foreign key (CREATED_BY)
         references USERS (USER_ID) on delete set null
);

create index LIB_ITEM_VERSIONS_IX1 on LIB_ITEM_VERSIONS (ITEM_ID);

create index LIB_ITEM_VERSIONS_IX2 on LIB_ITEM_VERSIONS (DATA_ID);

create index LIB_ITEM_VERSIONS_IX3 on LIB_ITEM_VERSIONS (CREATED_BY);

create table LIB_CURRENT_ITEMS (
    ITEM_ID char(36) not null,
    ...
    constraint LIB_CURRENT_ITEMS_FK1 foreign key (VERSION_ID)
        references LIB_ITEM_VERSIONS (VERSION_ID),
    constraint LIB_CURRENT_ITEMS_FK2 foreign key (ITEM_TYPE)
        references LIB_ITEM_TYPES (TYPE_ID),
    constraint LIB_CURRENT_ITEMS_FK3 foreign key (CREATED_BY)
        references USERS (USER_ID) on delete set null,
    constraint LIB_CURRENT_ITEMS_FK4 foreign key (MODIFIED_BY)
        references USERS (USER_ID) on delete set null,
    constraint LIB_CURRENT_ITEMS_FK5 foreign key (PARENT_ID)
        references LIB_CURRENT_ITEMS (ITEM_ID) on delete cascade
);

alter table LIB_CURRENT_ITEMS add constraint LIB_ITEMS_PARENT_NEQ_ITEM
    check (ITEM_ID != PARENT_ID);

create unique index LIB_CURRENT_ITEMS_UIX1 on LIB_CURRENT_ITEMS (upper(TITLE), ITEM_TYPE, PARENT_ID);

create index LIB_CURRENT_ITEMS_IX1 on LIB_CURRENT_ITEMS(CREATED_BY);

create index LIB_CURRENT_ITEMS_IX2 on LIB_CURRENT_ITEMS(MODIFIED_BY);

create index LIB_CURRENT_ITEMS_IX3 on LIB_CURRENT_ITEMS(ITEM_TYPE);

create index LIB_CURRENT_ITEMS_IX4 on LIB_CURRENT_ITEMS(upper(TITLE));

create index LIB_CURRENT_ITEMS_IX5 on LIB_CURRENT_ITEMS(PARENT_ID);

create index LIB_CURRENT_ITEMS_IX6 on LIB_CURRENT_ITEMS(VERSION_ID, ITEM_ID);

create view LIB_ITEMS as
select
    lci.ITEM_ID,
    lci.TITLE,
    lci.DESCRIPTION,
    ...
    lci.MODIFIED,
    lci.ACCESSED,
    COALESCE(ld.CONTENT_SIZE, 0) as CONTENT_SIZE,
    lci.PARENT_ID, lci.HIDDEN,
    lci.VERSION_ID,
    liv.DATA_ID
from LIB_CURRENT_ITEMS lci
left outer join LIB_ITEM_VERSIONS liv on lci.VERSION_ID = liv.VERSION_ID
left outer join LIB_DATA ld on liv.DATA_ID = ld.DATA_ID;
...
create table LIB_PROPERTIES_NONVER (
    ITEM_ID char(36) not null,
    PROPERTY_NAME varchar(150) not null,
    PROPERTY_VALUE varchar(256) not null,
    PROPERTY_BLOB_VALUE bytea null,
    constraint LIB_PROPERTIES_NONVER_FK1 foreign key (ITEM_ID)
        references LIB_CURRENT_ITEMS (ITEM_ID) on delete cascade
);

create index LIB_PROP_NONVER_IX1
    on LIB_PROPERTIES_NONVER (ITEM_ID, upper(PROPERTY_NAME), upper(PROPERTY_VALUE));

create index LIB_PROP_NONVER_IX2
    on LIB_PROPERTIES_NONVER (upper(PROPERTY_NAME), upper(PROPERTY_VALUE), ITEM_ID);

create table LIB_PROPERTIES_VER (
    VERSION_ID char(36) not null,
    PROPERTY_NAME varchar(150) not null,
    PROPERTY_VALUE varchar(256) not null,
    PROPERTY_BLOB_VALUE bytea null,
    constraint LIB_PROPERTIES_VER_FK1 foreign key (VERSION_ID)
        references LIB_ITEM_VERSIONS (VERSION_ID) on delete cascade
);

create index LIB_PROPERTIES_VER_IX1 on LIB_PROPERTIES_VER (VERSION_ID, upper(PROPERTY_NAME), upper(PROPERTY_VALUE));

create index LIB_PROPERTIES_VER_IX2 on LIB_PROPERTIES_VER (upper(PROPERTY_NAME), upper(PROPERTY_VALUE), VERSION_ID);

create view LIB_PROPERTIES as
select
    lci.ITEM_ID,
    lp.PROPERTY_NAME,
    lp.PROPERTY_VALUE,
    lp.PROPERTY_BLOB_VALUE
from LIB_CURRENT_ITEMS lci
inner join LIB_PROPERTIES_VER lp on lci.VERSION_ID = lp.VERSION_ID
union all select ITEM_ID, PROPERTY_NAME, PROPERTY_VALUE, PROPERTY_BLOB_VALUE from LIB_PROPERTIES_NONVER;
...
create table LIB_PRINCIPAL_PROPS (
    ...
    constraint FK_LPP_LIB_CURRENT_ITEMS foreign key (ITEM_ID)
        references LIB_CURRENT_ITEMS (ITEM_ID) on delete cascade,
...
create table LIB_RESOLVED_DEPEND_VER (
    DEPENDENT_VERSION_ID char(36) not null,
    REQUIRED_ID char(36) not null,
    DESCRIPTION varchar(1000) null,
    CASCADING_DELETE boolean not null,
    ORIGINAL_REQUIRED_ID char(36) null,
    constraint LIB_RESOLVED_DEPEND_PK primary key (DEPENDENT_VERSION_ID, REQUIRED_ID),
    constraint LIB_RESOLVED_DEPEND_FK1 foreign key (DEPENDENT_VERSION_ID)
        references LIB_ITEM_VERSIONS (VERSION_ID) on delete cascade,
    constraint LIB_RESOLVED_DEPEND_FK2 foreign key (REQUIRED_ID)
        references LIB_CURRENT_ITEMS (ITEM_ID) on delete no action
);

create index LIB_RESOLVED_INDEX1 on LIB_RESOLVED_DEPEND_VER (REQUIRED_ID);

create index LIB_RESOLVED_INDEX2 on LIB_RESOLVED_DEPEND_VER (DEPENDENT_VERSION_ID);

create view LIB_RESOLVED_DEPEND as
select
    lci.ITEM_ID as DEPENDENT_ID,
    lrd.REQUIRED_ID,
    lrd.DESCRIPTION,
    lrd.CASCADING_DELETE,
    lrd.ORIGINAL_REQUIRED_ID
from LIB_CURRENT_ITEMS lci
inner join LIB_RESOLVED_DEPEND_VER lrd on lci.VERSION_ID = lrd.DEPENDENT_VERSION_ID;

create table LIB_UNRESOLVED_DEPEND_VER (
    DEPENDENT_VERSION_ID char(36) not null,
    REQUIRED_ID char(36) not null,
    DESCRIPTION varchar(1000) null,
    CASCADING_DELETE boolean not null,
    ORIGINAL_REQUIRED_ID char(36) null,
    constraint LIB_UNRESOLVED_DEPEND_PK primary key (DEPENDENT_VERSION_ID, REQUIRED_ID),
    constraint LIB_UNRESOLVED_DEPEND_FK1 foreign key (DEPENDENT_VERSION_ID)
        references LIB_ITEM_VERSIONS (VERSION_ID) on delete cascade
);

create index LIB_UNRESOLVED_INDEX1 on LIB_UNRESOLVED_DEPEND_VER (REQUIRED_ID);

create index LIB_UNRESOLVED_INDEX2 on LIB_UNRESOLVED_DEPEND_VER (DEPENDENT_VERSION_ID);

create view LIB_UNRESOLVED_DEPEND as
select
    lci.ITEM_ID as DEPENDENT_ID,
    lud.REQUIRED_ID,
    lud.DESCRIPTION,
    lud.CASCADING_DELETE,
    lud.ORIGINAL_REQUIRED_ID
from LIB_CURRENT_ITEMS lci
inner join LIB_UNRESOLVED_DEPEND_VER lud on lci.VERSION_ID = lud.DEPENDENT_VERSION_ID;
...
create or replace function usp_copyItem(
    oldId in LIB_CURRENT_ITEMS.ITEM_ID%type,
    oldVersionId in LIB_CURRENT_ITEMS.VERSION_ID%type,
    newId in LIB_CURRENT_ITEMS.ITEM_ID%type,
    newVersionIdSuggestion in LIB_CURRENT_ITEMS.VERSION_ID%type,
    parentId in LIB_CURRENT_ITEMS.PARENT_ID%type,
    caller in LIB_CURRENT_ITEMS.MODIFIED_BY%type,
    newTitle in LIB_CURRENT_ITEMS.TITLE%type)
returns void as $$
declare
    timeOfUpdate timestamp(6);
    sourceVersionId char(36);
    targetVersionId char(36);
    cnt decimal;
begin
    select now() at time zone 'utc' into timeOfUpdate;

    if (oldVersionId is null) then
        -- We should copy the current version (if any)
        select VERSION_ID into sourceVersionId from LIB_CURRENT_ITEMS where ITEM_ID = oldId;
    else
        -- We should copy the given version
        select
            count(1) into cnt
        from LIB_ITEM_VERSIONS
        where ITEM_ID = oldId and VERSION_ID = oldVersionId;
        if (cnt = 0) then
          raise exception 'ERR-10007 - Version does not exist';
        end if;

        sourceVersionId := oldVersionId;
    end if;

    if (sourceVersionId is not null) then
        targetVersionId := newVersionIdSuggestion;
        insert into LIB_ITEM_VERSIONS (
            VERSION_ID,
            ITEM_ID,
            DATA_ID,
            FORMAT_VERSION,
            ...
            CREATED)
        select
            targetVersionId as VERSION_ID,
            newId as ITEM_ID,
            DATA_ID,
            FORMAT_VERSION,
            caller as CREATED_BY,
            timeOfUpdate as CREATED
        from LIB_ITEM_VERSIONS
    ...
    select
        newId as ITEM_ID,
        targetVersionId as VERSION_ID,
        coalesce (newTitle, TITLE) as TITLE,
        DESCRIPTION,
        ITEM_TYPE,
        CREATED_BY,
        CREATED,
...
create or replace function usp_moveItem(
    itemId LIB_CURRENT_ITEMS.ITEM_ID%type,
    newParentId LIB_CURRENT_ITEMS.PARENT_ID%type,
    caller LIB_CURRENT_ITEMS.MODIFIED_BY%type,
    newTitle LIB_CURRENT_ITEMS.TITLE%type)
returns void as $$
declare
    timeOfUpdate timestamp(6);
begin
...
returns table(
    ITEM_ID LIB_CURRENT_ITEMS.ITEM_ID%type,
    TITLE LIB_CURRENT_ITEMS.TITLE%type ,
    DESCR LIB_CURRENT_ITEMS.DESCRIPTION%type,
    ITEM_TYPE LIB_CURRENT_ITEMS.ITEM_TYPE%type,
    FORMAT_VERSION LIB_ITEM_VERSIONS.FORMAT_VERSION%type,
    CREATED_BY LIB_CURRENT_ITEMS.CREATED_BY%type,
    CREATED LIB_CURRENT_ITEMS.CREATED%type,
    MODIFIED_BY LIB_CURRENT_ITEMS.MODIFIED_BY%type,
    MODIFIED LIB_CURRENT_ITEMS.MODIFIED%type,
    ACCESSED LIB_CURRENT_ITEMS.ACCESSED%type,
    CONTENT_SIZE LIB_DATA.CONTENT_SIZE%type,
    PARENT_ID LIB_CURRENT_ITEMS.PARENT_ID%type,
    HIDDEN LIB_CURRENT_ITEMS.HIDDEN%type,
    VERSION_ID LIB_CURRENT_ITEMS.VERSION_ID%type,
    DATA_ID LIB_DATA.DATA_ID%type) as $$
begin

    -- Properties
    insert into LIB_PROPERTIES_NONVER (
        ITEM_ID,
        PROPERTY_NAME,
        PROPERTY_VALUE,
        PROPERTY_BLOB_VALUE)
    select 
        cm.COPY_ITEM_ID as ITEM_ID,
        PROPERTY_NAME,
        PROPERTY_VALUE,
        PROPERTY_BLOB_VALUE
    from LIB_PROPERTIES_NONVER original, LIB_COPY_MAPPING cm
    where original.ITEM_ID = cm.ORIGINAL_ITEM_ID;

    insert into LIB_PROPERTIES_VER (
        VERSION_ID,
        PROPERTY_NAME,
        PROPERTY_VALUE,
        PROPERTY_BLOB_VALUE)
    select
        cm.COPY_VERSION_ID as VERSION_ID,
        PROPERTY_NAME,
        PROPERTY_VALUE,
        PROPERTY_BLOB_VALUE
    from LIB_PROPERTIES_VER original, LIB_COPY_MAPPING cm
    where original.VERSION_ID = cm.ORIGINAL_VERSION_ID;
    ...
    insert into LIB_UNRESOLVED_DEPEND_VER (
    ...
    select
        cm.COPY_VERSION_ID as DEPENDENT_VERSION_ID,
        REQUIRED_ID,
        DESCRIPTION,
        CASCADING_DELETE,
        ORIGINAL_REQUIRED_ID
    from LIB_UNRESOLVED_DEPEND_VER original, LIB_COPY_MAPPING cm
    where original.DEPENDENT_VERSION_ID = cm.ORIGINAL_VERSION_ID;

    -- Resolved dependencies where the required items also have been copied and there is no original required ID
    insert into LIB_RESOLVED_DEPEND_VER (
        DEPENDENT_VERSION_ID,
        REQUIRED_ID,
        DESCRIPTION,
        CASCADING_DELETE,
        ORIGINAL_REQUIRED_ID)
    select
        dep.COPY_VERSION_ID as DEPENDENT_VERSION_ID,
        req.COPY_ITEM_ID as REQUIRED_ID,
        original.DESCRIPTION,
        original.CASCADING_DELETE,
        original.REQUIRED_ID
    from LIB_RESOLVED_DEPEND_VER original, LIB_COPY_MAPPING dep, LIB_COPY_MAPPING req
    where original.DEPENDENT_VERSION_ID = dep.ORIGINAL_VERSION_ID
        and original.REQUIRED_ID = req.ORIGINAL_VERSION_ID
    ...
    select
        ...
        original.CASCADING_DELETE,
        original.ORIGINAL_REQUIRED_ID
    from LIB_RESOLVED_DEPEND_VER original, LIB_COPY_MAPPING dep
    where original.DEPENDENT_VERSION_ID = dep.ORIGINAL_VERSION_ID
        and not exists (select 1
                        from LIB_COPY_MAPPING
                        where ORIGINAL_VERSION_ID = original.REQUIRED_ID);
    ...
    return query
        ...
        where i.ITEM_ID = cm.COPY_ITEM_ID;
end;
$$
language plpgsql;


create or replace function usp_insertDependency(
    dependentId in LIB_RESOLVED_DEPEND_VER.DEPENDENT_VERSION_ID%type,
    requiredId in LIB_RESOLVED_DEPEND_VER.REQUIRED_ID%type,
    description in LIB_RESOLVED_DEPEND_VER.DESCRIPTION%type,
    cascadingDelete in LIB_RESOLVED_DEPEND_VER.CASCADING_DELETE%type,
    originalRequiredId in LIB_RESOLVED_DEPEND_VER.ORIGINAL_REQUIRED_ID%type)
returns void as $$
declare
  cnt decimal;
begin
    select count(*) into cnt from LIB_CURRENT_ITEMS where ITEM_ID = requiredId;
...
begin
    ...
    if (administrationEnabled)
        ...
        with recursive PERMISSIONS_ON_ITEM_CTE as (
            ...
            union all
            select li.PARENT_ID
            from PERMISSIONS_ON_ITEM_CTE poi, LIB_CURRENT_ITEMS li
...
begin
    ...
    for descendant in
        ...
        with recursive ITEM_PARENT_CTE (ITEM_ID, PARENT_ID) as (
            ...
            select
                li.ITEM_ID,
                li.PARENT_ID
            from LIB_CURRENT_ITEMS li, ITEM_PARENT_CTE cte
    ...
    loop
        ...
        with recursive PERMISSIONS_ON_ITEM_CTE as (
            ...
            union all
            select i.PARENT_ID
            from PERMISSIONS_ON_ITEM_CTE poi, LIB_CURRENT_ITEMS i
...
create or replace function usp_insertItem(
    newVersionId  LIB_CURRENT_ITEMS.VERSION_ID%type,
    newItemId  LIB_CURRENT_ITEMS.ITEM_ID%type,
    newTitle  LIB_CURRENT_ITEMS.TITLE%type,
    newDescription  LIB_CURRENT_ITEMS.DESCRIPTION%type,
    newItemType  LIB_CURRENT_ITEMS.ITEM_TYPE%type,
    newDataId  LIB_ITEM_VERSIONS.DATA_ID%type,
    newFormatVersion  LIB_ITEM_VERSIONS.FORMAT_VERSION%type,
    newParentId  LIB_CURRENT_ITEMS.PARENT_ID%type,
    newHidden  LIB_CURRENT_ITEMS.HIDDEN%type,
    caller  LIB_CURRENT_ITEMS.CREATED_BY%type)
returns void as $$
declare
    hasAccess boolean;
...
begin
    ...
    select count(1) into cnt from LIB_CURRENT_ITEMS where ITEM_ID = newParentId;
    if (cnt = 0) then
    ...
    into isContainer
    from LIB_ITEM_TYPES it, LIB_CURRENT_ITEMS i
    ...
    with recursive CTE (LVL, ITEM_ID) as (
        ...
        select
            c.LVL + 1,
            i.PARENT_ID
        from LIB_CURRENT_ITEMS i, CTE c
    ...
    if (newVersionId is not null) then
    ...
    (select
        ...
        CASCADING_DELETE,
        ORIGINAL_REQUIRED_ID
    from LIB_UNRESOLVED_DEPEND_VER
    where REQUIRED_ID = newItemId);

    delete from LIB_UNRESOLVED_DEPEND_VER
    where REQUIRED_ID = newItemId;
...
create or replace function usp_updateItem(
    itemId LIB_CURRENT_ITEMS.ITEM_ID%type,
    versionId LIB_CURRENT_ITEMS.VERSION_ID%type,
    newTitle LIB_CURRENT_ITEMS.TITLE%type,
    newDescription LIB_CURRENT_ITEMS.DESCRIPTION%type,
    newDataId LIB_ITEM_VERSIONS.DATA_ID%type,
    newFormatVersion LIB_ITEM_VERSIONS.FORMAT_VERSION%type,
...
begin
    ...
    select count(1) into cnt from LIB_CURRENT_ITEMS where ITEM_ID = itemId;
    if (cnt = 0) then
...
declare
    ...
    cnt decimal;
    parentId LIB_CURRENT_ITEMS.PARENT_ID%type;
    timeOfDeletion LIB_CURRENT_ITEMS.MODIFIED%type;
begin
    ...
    insert into LIB_TEMP_DESCENDANTS (
        ...
        DESCENDANT_ID,
        DESCENDANT_DATA_ID)
    with recursive cte (ITEM_ID, PARENT_ID, DATA_ID) as (
        select
            ITEM_ID,
            PARENT_ID,
            DATA_ID
        ...
        where ITEM_ID = itemId
        union all
        select item.ITEM_ID, item.PARENT_ID, item.DATA_ID
        from LIB_ITEMS item, cte parent
        where item.PARENT_ID = parent.ITEM_ID
      ) select itemId, ITEM_ID, DATA_ID from cte;
    ...
    insert into LIB_UNRESOLVED_DEPEND_VER (
        ...
        ORIGINAL_REQUIRED_ID)
    (select
        rd.DEPENDENT_VERSION_ID,
        rd.REQUIRED_ID,
        rd.DESCRIPTION,
        rd.CASCADING_DELETE,
        rd.ORIGINAL_REQUIRED_ID
    from LIB_RESOLVED_DEPEND_VER rd, LIB_TEMP_DESCENDANTS ds
    where rd.CASCADING_DELETE = '0'
        and rd.REQUIRED_ID = ds.DESCENDANT_ID
        and ds.ANCESTOR_ID = itemId
    );
    ...
    delete from LIB_RESOLVED_DEPEND_VER
    where REQUIRED_ID in (
        select DESCENDANT_ID
        from LIB_TEMP_DESCENDANTS
        where ANCESTOR_ID = itemId);

  -- Check if not analysisstate
  select count(1) into cnt
    from LIB_CURRENT_ITEMS
    where ITEM_ID = itemId
    and ITEM_TYPE != '04a1dc4d-6794-4c49-b91b-063de8a77450';

    -- Delete the item itself and any descendants
    delete from LIB_CURRENT_ITEMS
    where ITEM_ID in (
        select DESCENDANT_ID
        from LIB_TEMP_DESCENDANTS
        where ANCESTOR_ID = itemId);

    -- Keep the data ID of all deleted versions
   insert into LIB_TEMP_DELETED_DATA_REFS (DATA_ID) (select DATA_ID from LIB_ITEM_VERSIONS where ITEM_ID in
    (select DESCENDANT_ID from LIB_TEMP_DESCENDANTS where ANCESTOR_ID = itemId));

    -- Delete all versions of the item itself and any descendants
    delete from LIB_ITEM_VERSIONS
    where ITEM_ID in
        (select
            DESCENDANT_ID
        from LIB_TEMP_DESCENDANTS
    ...
    -- Touch the parent, if not analysisstate
    if (cnt > 0) then
	    update LIB_CURRENT_ITEMS
	    set
	        MODIFIED_BY = caller,
...
begin
    ...
    delete from LIB_DATA where DATA_ID in
    (select DATA_ID from LIB_TEMP_DELETED_DATA_REFS 
    where not exists (select 1 from LIB_ITEM_VERSIONS where LIB_ITEM_VERSIONS.DATA_ID = LIB_TEMP_DELETED_DATA_REFS.DATA_ID));
      
    return query select DATA_ID 
    from LIB_TEMP_DELETED_DATA_REFS ltddr
    where not exists (select 1 from LIB_DATA ld where ltddr.DATA_ID = ld.DATA_ID);
    
    drop table LIB_TEMP_DESCENDANTS;
    drop table LIB_TEMP_DELETED_DATA_REFS;
end;
$$
language plpgsql;

create or replace function usp_restoreVersion (
  itemId in LIB_CURRENT_ITEMS.ITEM_ID%type,
  versionId in LIB_CURRENT_ITEMS.VERSION_ID%type,
  newVersionId in LIB_CURRENT_ITEMS.VERSION_ID%type,
  caller in LIB_CURRENT_ITEMS.MODIFIED_BY%type)
returns void as $$
declare
  timeOfUpdate timestamp(6);
begin
  ...
  insert into LIB_ITEM_VERSIONS (VERSION_ID, ITEM_ID, DATA_ID, FORMAT_VERSION, CREATED_BY, CREATED, 
    RESTORED_FROM_ID, RESTORED_FROM_TIMESTAMP)
  select newVersionId as VERSION_ID, ITEM_ID, DATA_ID, FORMAT_VERSION, caller as CREATED_BY, timeOfUpdate as CREATED, 
    versionId as RESTORED_FROM_ID, CREATED as RESTORED_FROM_TIMESTAMP
  from LIB_ITEM_VERSIONS where VERSION_ID = versionId and ITEM_ID = itemId;
  if not found then
    raise exception 'ERR-10007 - Version does not exist';
  end if;

  -- Copy versioned properties
  insert into LIB_PROPERTIES_VER (VERSION_ID, PROPERTY_NAME, PROPERTY_VALUE, PROPERTY_BLOB_VALUE)
    select newVersionId as VERSION_ID, PROPERTY_NAME, PROPERTY_VALUE, PROPERTY_BLOB_VALUE
    from LIB_PROPERTIES_VER where VERSION_ID = versionId;

  -- Copy unresolved dependencies
  insert into LIB_UNRESOLVED_DEPEND_VER (DEPENDENT_VERSION_ID, REQUIRED_ID, DESCRIPTION, CASCADING_DELETE, ORIGINAL_REQUIRED_ID)
    select newVersionId as DEPENDENT_VERSION_ID, REQUIRED_ID, DESCRIPTION, CASCADING_DELETE, ORIGINAL_REQUIRED_ID
    from LIB_UNRESOLVED_DEPEND_VER where DEPENDENT_VERSION_ID = versionId;

  -- Copy resolved dependencies
  insert into LIB_RESOLVED_DEPEND_VER (DEPENDENT_VERSION_ID, REQUIRED_ID, DESCRIPTION, CASCADING_DELETE, ORIGINAL_REQUIRED_ID)
    select newVersionId as DEPENDENT_VERSION_ID, REQUIRED_ID, DESCRIPTION, CASCADING_DELETE, ORIGINAL_REQUIRED_ID
    from LIB_RESOLVED_DEPEND_VER where DEPENDENT_VERSION_ID = versionId;

  update LIB_CURRENT_ITEMS set VERSION_ID = newVersionId, MODIFIED_BY = caller, MODIFIED = timeOfUpdate where ITEM_ID = itemId;
  if not found then
    raise exception 'ERR-10005 - Item does not exist';
  end if;
end;
...
-- Stored procedure used for deleting item versions
create or replace function usp_deleteVersion (
  itemId LIB_ITEM_VERSIONS.ITEM_ID%type,
  versionId LIB_ITEM_VERSIONS.VERSION_ID%type,
  caller LIB_CURRENT_ITEMS.MODIFIED_BY%type)
returns void as $$
declare
  currentVersionId char(36);
  previousVersionId char(36);
  itemIdOfVersion char(36);
  dataIdOfVersion char(36);
begin
  select ITEM_ID, DATA_ID into itemIdOfVersion, dataIdOfVersion from LIB_ITEM_VERSIONS where VERSION_ID = versionId;
  if NOT FOUND then
    -- The version does not exist (which is what we wanted to accomplish)
    return;
  end if;

  if (itemId != itemIdOfVersion) then 
    -- Trying to delete a version that belongs to another item than the given one
    raise exception 'ERR-10005 - Item does not exist';
  end if;

  select VERSION_ID into currentVersionId from LIB_CURRENT_ITEMS where ITEM_ID = itemId;
  if NOT FOUND then
  ...
  if (versionId = currentVersionId) then
    select VERSION_ID into previousVersionId from LIB_ITEM_VERSIONS 
      where ITEM_ID = itemId
      and CREATED < (select CREATED from LIB_ITEM_VERSIONS where VERSION_ID = versionId)
      order by CREATED desc limit 1;

    if (previousVersionId is null) then
      raise exception 'ERR-10004 - Cannot delete the last version';
    end if;

    update LIB_CURRENT_ITEMS set VERSION_ID = previousVersionId, 
      MODIFIED_BY = caller, MODIFIED = now() at time zone 'utc' where ITEM_ID = itemId;
  end if;

  -- Keep the data ID of the deleted version
  insert into LIB_TEMP_DELETED_DATA_REFS (DATA_ID) values (dataIdOfVersion);

  -- Delete the version
  delete from LIB_ITEM_VERSIONS where ITEM_ID = itemId and VERSION_ID = versionId;
...
create table TRUSTED_CERTS (
  TRUSTED_ROLE varchar(60) not null,
...
create table CONSENTED_ENTITIES (
   CONSENTED_ENTITY_HASH char(24) not null,
   CONSENTING_USER_ID char(36) not null,
   LIBRARY_ITEM_ID char(36) not null,
   CONSENTED_ENTITY bytea not null,
   ADDED_DATE timestamp null,
   constraint CONSENTED_ENTITIES_PK primary key (CONSENTED_ENTITY_HASH, CONSENTING_USER_ID, LIBRARY_ITEM_ID),
   constraint FK_CONSENTED_ENTITIES1 foreign key (CONSENTING_USER_ID) 
      references USERS (USER_ID)  on delete cascade,
   constraint FK_CONSENTED_ENTITIES2 foreign key (LIBRARY_ITEM_ID)
      references LIB_CURRENT_ITEMS (ITEM_ID) on delete cascade);

create index CONSENTED_ENTITIES_IX on CONSENTED_ENTITIES (
   CONSENTING_USER_ID,
   LIBRARY_ITEM_ID);

create index CONSENTED_ENTITIES_IX1 on CONSENTED_ENTITIES (CONSENTING_USER_ID);

create index CONSENTED_ENTITIES_IX2 on CONSENTED_ENTITIES (LIBRARY_ITEM_ID);
...
create index TRUSTED_USERS_IX3 on TRUSTED_USERS (TRUSTED_USER_ID);
...
create index TRUSTED_CERTS_IX3 on TRUSTED_CERTS (TRUSTED_ISSUER, TRUSTED_SERIAL_NUMBER);
...
alter table ROUTING_RULES add constraint FK_ROUT_RULES_LIB_ITEMS foreign key(LIB_ITEM_ID)
    references LIB_CURRENT_ITEMS(ITEM_ID) on delete set null;
...
create or replace function usp_findRoutingRules(
    ...
    current_library_item_id LIB_CURRENT_ITEMS.ITEM_ID%type,
...
begin
    ...
    return query
        ...
        ALL_PARENTS (ITEM_ID) as (
            select PARENT_ID from LIB_CURRENT_ITEMS
            WHERE ITEM_ID = coalesce(current_library_item_id,'')
            union all
            select items.PARENT_ID
            from LIB_CURRENT_ITEMS items, ALL_PARENTS cte
...
create table INVITES (
    ...
    constraint FK_INVITES_LIB_ITEMS foreign key (ITEM_ID) references LIB_CURRENT_ITEMS (ITEM_ID) on delete cascade
);
...
CREATE TABLE ACTIVITY  (
  ...
  CONSTRAINT ACTIVITY_PK_ID PRIMARY KEY (activity_id)

);

CREATE UNIQUE INDEX ACTIVITY_PK ON ACTIVITY (REFERENCE_ID, ACTIVITY_TYPE, STATE) 
...
create index DDC_PUBLISHED_BY_IX1 on DEP_DISTRIBUTION_CONTENTS (PUBLISHED_BY);
